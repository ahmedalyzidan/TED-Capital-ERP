...
class ProjectController {
    ...
    async recordCollection(req, res) {
        ...
        try {
            ...
            if (valuation_id && !isNaN(parseInt(valuation_id)) && !String(valuation_id).startsWith('val-')) {
                const invRes = await client.query("SELECT total_amount FROM ar_invoices WHERE id = $1::integer", [parseInt(valuation_id)]);
                if (invRes.rows.length > 0) {
                    ...
                    `, [parseInt(valuation_id)]);
                    const totalPaid = parseFloat(paidRes.rows[0].total_paid || 0);

                    const newStatus = totalPaid >= totalAmount ? 'Paid' : 'Partially Paid';
                    await client.query("UPDATE ar_invoices SET status = $1 WHERE id = $2::integer", [newStatus, parseInt(valuation_id)]);
                }
            }
