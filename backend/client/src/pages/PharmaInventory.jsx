                    ...
                    <div className="bg-slate-950 p-6 text-white border-b border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6 shrink-0 relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-4 relative z-10 w-full md:w-auto justify-end">
                        ...
                        <button
                          ...
                          className={`px-6 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl flex items-center gap-2 ${isIotLive
              ...
              <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
                ...
                <div className="relative z-10">
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/20 border border-amber-500/30 rounded-xl text-amber-300 text-xs font-black uppercase tracking-widest mb-3">
                    <span>🛒</span> {language === 'ar' ? 'سجل الحركات والمبيعات الدوائية (Pharma Sales & Dispense Ledger)' : 'Pharma Sales & Dispense Ledger'}
                  </div>
                  <h2 className="text-3xl font-black tracking-tight text-white">{language === 'ar' ? 'حصر ومراقبة المبيعات وحركات الصرف' : 'Sales & Dispensation Monitoring'}</h2>
                  <p className="text-xs text-slate-300 font-bold mt-2 max-w-2xl">
                    {language === 'ar' ? 'سجل فوري ومفصل يعرض كافة الحركات الصادرة من الصيدلية والمستودع الطبي، مشمولاً بإجمالي القيم المالية، طرق السداد، وحالة التوثيق الرقابي.' : 'Instant, detailed ledger showing all outgoing pharmacy and warehouse transactions, including financial values, payment methods, and compliance documentation.'}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-4 relative z-10 w-full lg:w-auto">
                  <div className="bg-white/10 backdrop-blur-md border border-white/10 p-5 rounded-2xl flex-1 min-w-[160px]">
                    <span className="text-[11px] font-bold text-slate-300 block">{language === 'ar' ? 'إجمالي قيمة الصرف والمبيعات' : 'Total Sales & Dispense Value'}</span>
                    <span className="text-2xl font-black text-amber-400 font-mono mt-1 block">
                      {salesLogs.reduce((sum, item) => sum + Number(item.total_amount || 0), 0).toLocaleString()} EGP
                    </span>
                  </div>
                  <div className="bg-white/10 backdrop-blur-md border border-white/10 p-5 rounded-2xl flex-1 min-w-[160px]">
                    <span className="text-[11px] font-bold text-slate-300 block">{language === 'ar' ? 'عدد العبوات المنصرفة' : 'Dispensed Packs'}</span>
                    <span className="text-2xl font-black text-white font-mono mt-1 block">
                      {salesLogs.reduce((sum, item) => sum + Number(item.qty || 0), 0).toLocaleString()} {language === 'ar' ? 'عبوة' : 'Packs'}
                    </span>
                  </div>
                  <div className="bg-white/10 backdrop-blur-md border border-white/10 p-5 rounded-2xl flex-1 min-w-[140px]">
                    <span className="text-[11px] font-bold text-slate-300 block">{language === 'ar' ? 'إجمالي عدد العمليات' : 'Total Operations'}</span>
                    <span className="text-2xl font-black text-cyan-400 font-mono mt-1 block">
                      {salesLogs.length} {language === 'ar' ? 'عملية' : 'Ops'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-[2rem] border border-slate-150 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
                <div className="w-full md:w-96 relative">
                  ...
                  <input
                    ...
                    placeholder={language === 'ar' ? 'بحث باسم الصنف، المريض، الطبيب، أو رقم الحركة...' : 'Search item name, patient, doctor, or sale no...'}
                    value={salesSearch}
                    onChange={(e) => setSalesSearch(e.target.value)}
                  />
                </div>
                <div className="text-xs text-slate-500 font-bold flex items-center gap-2">
                  <span>💡 {language === 'ar' ? 'تلميح:' : 'Tip:'}</span>
                  <span>{language === 'ar' ? 'كافة حركات الصرف مرتبطة آلياً بالقيود المحاسبية وسجل عهدة الأدوية المراقبة.' : 'All dispensing movements are automatically linked to accounting entries and controlled drug custody ledgers.'}</span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2rem] border border-slate-150 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                  <h3 className="text-sm font-black text-slate-800 flex items-center gap-2">
                    <span>📋</span> {language === 'ar' ? 'تفاصيل حركات المبيعات والصرف' : 'Sales & Dispense Movement Details'}
                  </h3>
                  <span className="text-xs font-bold text-slate-500">
                    {language === 'ar' ? 'عدد النتائج:' : 'Results:'} {salesLogs.filter(item =>
                      !salesSearch ||
                  ...
                  <table className="w-full text-right border-collapse" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                    <thead>
                      <tr className="bg-slate-100/80 text-slate-600 text-[10px] font-black uppercase tracking-widest border-b border-slate-200">
                        <th className="p-4">{language === 'ar' ? 'رقم الحركة' : 'Sale No'}</th>
                        <th className="p-4">{language === 'ar' ? 'الصنف الدوائي والباتش' : 'Drug Item & Batch'}</th>
                        <th className="p-4 text-center">{language === 'ar' ? 'الكمية' : 'Qty'}</th>
                        <th className="p-4 text-center">{language === 'ar' ? 'سعر الوحدة' : 'Unit Price'}</th>
                        <th className="p-4 text-center">{language === 'ar' ? 'إجمالي القيمة' : 'Total Value'}</th>
                        <th className="p-4">{language === 'ar' ? 'الجهة / المريض' : 'Patient / Clinic'}</th>
                        <th className="p-4">{language === 'ar' ? 'الطبيب المسئول' : 'Doctor'}</th>
                        <th className="p-4 text-center">{language === 'ar' ? 'طريقة السداد' : 'Payment Method'}</th>
                        <th className="p-4 text-center">{language === 'ar' ? 'التاريخ' : 'Date'}</th>
                        <th className="p-4 text-center">{language === 'ar' ? 'إجراءات أمنية' : 'Security'}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 text-xs font-bold text-slate-700">
                      {salesLogs.filter(item =>
                        ...
                        <tr>
                          <td colSpan="10" className="text-center py-16 text-slate-400 font-bold text-sm">
                            {language === 'ar' ? 'لا توجد حركات مبيعات أو صرف مطابقة للبحث.' : 'No sales or dispense movements match your search.'}
                          </td>
                        </tr>
                      ) : (
                        salesLogs.filter(item =>
                          ...
                          <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td className="p-4 text-center">
                              <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black border ${item.payment_method?.includes('تأمين')
