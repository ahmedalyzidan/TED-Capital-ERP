  ...
  return (
    <div className="space-y-10 animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <StatCard title={cur.stats.total} value={summary.totalAmount} type="primary" icon="💰" />
         <StatCard title={cur.stats.tax} value={summary.totalTax} type="secondary" icon="⚖️" />
         <StatCard title={cur.stats.count} value={summary.count} type="neutral" icon="📊" suffix="" />
                  ...
                  {analytics.projectCosts.map((p, i) => (
                     <div key={i} className="space-y-2">
                        <div className="flex justify-between text-xs font-black">
                           ...
                           <span className="text-white font-mono">{Number(p.total_cost).toLocaleString()}</span>
                        </div>
                        <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                           <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${(p.total_cost / analytics.projectCosts[0].total_cost) * 100}%` }}></div>
                        </div>
                     </div>
                  ))}
               </div>
         ...
         <div className="overflow-x-auto custom-scrollbar">
            <table className={`w-full ${language === 'ar' ? 'text-right' : 'text-left'} whitespace-nowrap`}>
               ...
               <tbody className="divide-y divide-slate-50">
                  {loading ? (
                     <tr><td colSpan="6" className="p-20 text-center text-slate-300 font-bold animate-pulse">Synchronizing ledger data...</td></tr>
                  ) : expenses.map(exp => (
                     <tr 
                        key={exp.id} 
                     ...
                     >
                        ...
                        <td className="px-10 py-6">
                           <div className="flex flex-col">
                              <span className="font-black text-slate-900 group-hover:text-slate-600 transition-colors">{exp.description}</span>
                              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tight mt-1">{exp.supplier_name || 'N/A'}</span>
                           </div>
                        </td>
                        <td className="px-10 py-6">
                           <span className="px-3 py-1 bg-slate-100 text-slate-900 rounded-lg text-[10px] font-black border border-slate-200">{exp.category_name}</span>
                        </td>
                        <td className="px-10 py-6">
                           {exp.project_name ? (
                        ...
                        <td className={`px-10 py-6 ${language === 'ar' ? 'text-left' : 'text-right'}`}>
                           <div className="flex flex-col">
                              <span className="font-black text-slate-900 font-mono text-lg tracking-tighter">{Number(exp.amount).toLocaleString()}</span>
            ...
            <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl relative z-10 overflow-hidden animate-scale-up border border-white/10 flex flex-col max-h-[90vh]">
              <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                 <div className="flex items-center gap-6">
                    ...
                    <h3 className="text-2xl font-black text-slate-900 tracking-tighter uppercase">{cur.form.title}</h3>
                 </div>
              ...
              <form onSubmit={handleSubmit} className="p-10 space-y-10 max-h-[70vh] overflow-y-auto custom-scrollbar">
                 ...
                 <button type="submit" disabled={isSubmitting} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black text-xl hover:bg-slate-800 transition-all shadow-2xl shadow-slate-900/30 flex items-center justify-center gap-4 active:scale-[0.98]">
                    {isSubmitting ? <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin"></div> : <>{cur.form.save}</>}
                 </button>
              </form>
           </div>
        </div>
            ...
            <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl relative z-10 overflow-hidden border border-white/20 animate-in slide-in-from-bottom-10 duration-500 flex flex-col max-h-[90vh]">
               <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                  <div className="flex items-center gap-8">
                     ...
                     <div>
                        <h3 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">{detailModal.expense.description}</h3>
               ...
               <div className="p-12 overflow-y-auto custom-scrollbar flex-1 bg-white space-y-16">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                     <div className="bg-slate-900 p-10 rounded-[2.5rem] text-white shadow-xl shadow-slate-900/20">
                        ...
                        <p className="text-4xl font-black font-mono tracking-tighter">{(Number(detailModal.expense.amount) + Number(detailModal.expense.tax_amount)).toLocaleString()}</p>
                     ...
                     <div className="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-200">
                        ...
                        <p className="text-2xl font-black text-slate-900 uppercase tracking-tight">{detailModal.expense.category_name}</p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                     <div className="space-y-10">
                        ...
                        <div className="space-y-8">
                           ...
                           <MetaRow label="Payment Vector" value={detailModal.expense.metadata?.payment_detail || detailModal.expense.payment_method} />
                           ...
                           <MetaRow label="Audit Lead" value={detailModal.expense.creator_name} />
                        </div>
                     </div>
                     ...
                     <div className="bg-slate-950 rounded-[3rem] p-12 text-white relative overflow-hidden group">
                        ...
                        <div className="flex flex-col items-center justify-center h-[200px] border-2 border-dashed border-white/5 rounded-[2rem] gap-4">
                           ...
                           <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">No Receipt Linked to this Node</p>
                           <button className="text-[10px] font-black text-emerald-400 uppercase tracking-widest hover:text-emerald-300 transition-colors">Attach Documentation →</button>
                        </div>
                     </div>
                  </div>
               </div>
    ...
    <div className={`p-8 rounded-[2.5rem] border shadow-sm transition-all hover:shadow-xl hover:shadow-slate-200/50 group relative overflow-hidden ${colors[type]}`}>
       ...
       <div className="flex flex-col relative z-10">
          ...
          <div className="flex items-baseline gap-3">
            <span className={`text-4xl font-black font-mono tracking-tighter ${textColors[type]}`}>{Number(value).toLocaleString()}</span>
