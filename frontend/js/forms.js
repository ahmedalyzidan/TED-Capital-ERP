...
document.getElementById('dynamicForm').onsubmit = async (e) => {
    ...
    // 🚀 التحقق من الرصيد الدائن للعميل قبل حفظ المبيعات للتسوية الآلية
    let deductCreditFlag = false;
    if (type === 'inventory_sales' && !window.editingId) {
        const custName = data.customer_name;
        const clientCons = (window.erpData.client_consumptions_all || window.erpData.client_consumptions || []).filter(c => c.client_name === custName);
        let tPaid = 0, tRev = 0;
        clientCons.forEach(c => {
            tPaid += parseFloat(c.paid_amount || 0);
            tRev += parseFloat(c.total_revenue || 0);
        });
        const creditBal = Math.max(0, tPaid - tRev);

        if (creditBal > 0) {
            const deduct = confirm(`العميل (${custName}) لديه رصيد دائن مسبق بقيمة ${window.formatMoney ? window.formatMoney(creditBal) : creditBal}.\n\nهل ترغب في تسوية هذه الفاتورة الجديدة من رصيده الدائن آلياً؟`);
            if (deduct) {
                deductCreditFlag = true;
            }
        }
    }

    let url = window.editingId ? `/api/update/${type}/${window.editingId}` : `/api/add/${type}`;
    if (deductCreditFlag) {
        url += `?deduct_credit=true`; // تمرير الأمر في الرابط لتجنب خطأ الداتابيز
    }
...
window.autoDistributeInstallments = function () {
    const totalAmount = parseFloat(document.getElementById('scheduleTotalAmount')?.value || 0);
    const months = parseInt(document.getElementById('auto_dist_months')?.value || 1);
    const startDateVal = document.getElementById('auto_dist_start_date')?.value;

    if (!startDateVal) { alert("الرجاء اختيار تاريخ أول قسط أولاً."); return; }
    if (totalAmount <= 0) { alert("لا يوجد مبلغ مديونية لجدولته."); return; }
    ...
    for (let i = 0; i < months; i++) {
        if (typeof window.addScheduleRow === 'function') window.addScheduleRow();

        const rows = document.querySelectorAll('.schedule-row');
        if (rows.length > 0) {
            const lastRow = rows[rows.length - 1];
            const amtInput = lastRow.querySelector('.schedule-amount');
            const dateInput = lastRow.querySelector('.schedule-date');

            if (amtInput) amtInput.value = installmentAmount;
            if (dateInput) dateInput.value = currentDate.toISOString().split('T')[0];
        }
        currentDate.setMonth(currentDate.getMonth() + 1);
    }

    // محاكاة حدث الإدخال لتحديث المجاميع
    const firstInput = document.querySelector('.schedule-amount');
    if (firstInput) firstInput.dispatchEvent(new Event('input', { bubbles: true }));
    alert(`✅ تم توزيع المبلغ على ${months} قسط آلياً.`);
};
...
window.seedDefaultGLMappings = async function () {
    if (!confirm("هل تريد تحميل كافة قيود الربط المحاسبي الشاملة للنظام؟ سيؤدي ذلك لضبط إعدادات التوجيه المالي آلياً لجميع الحركات بما فيها الرواتب.")) return;
    ...
    const defaultMappings = [
        ...
        { transaction_type: 'Delayed Payment', debit_account: 'الخزينة/البنك', credit_account: 'العملاء', cost_center_required: true },
        { transaction_type: 'Partner Deposit', debit_account: 'الخزينة/البنك', credit_account: 'جاري الشركاء', cost_center_required: true },
        { transaction_type: 'Partner Withdrawal', debit_account: 'جاري الشركاء', credit_account: 'الخزينة/البنك', cost_center_required: true },
        { transaction_type: 'Client Refund', debit_account: 'العملاء', credit_account: 'الخزينة/البنك', cost_center_required: true },
        { transaction_type: 'PO DDP Charge', debit_account: 'مصاريف نقل وتخليص', credit_account: 'الخزينة/البنك', cost_center_required: true },
        { transaction_type: 'Return to Vendor', debit_account: 'الموردين', credit_account: 'المخزن الرئيسي', cost_center_required: true },
        { transaction_type: 'Payroll Payment', debit_account: 'مصاريف رواتب وأجور', credit_account: 'الخزينة/البنك', cost_center_required: true }
    ];

    const btn = document.querySelector('button[onclick="window.seedDefaultGLMappings()"]');
    if (btn) { btn.disabled = true; btn.innerText = "جاري التحميل ⏳..."; }

    try {
        for (const mapping of defaultMappings) {
            await window.apiFetch('/api/add/gl_mappings', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(mapping)
            });
        }
        alert("✅ تم الانتهاء من تسجيل كافة القيود المحاسبية الشاملة (13 قيد) بنجاح.");
        if (typeof window.safeSystemRefresh === 'function') window.safeSystemRefresh('gl_mappings');
    } catch (e) {
        console.error("GL Mapping Seed Error:", e);
        alert("❌ حدث خطأ أثناء تحميل القيود. تأكد من اتصالك بالإنترنت وصلاحية الجلسة.");
    } finally {
        if (btn) { btn.disabled = false; btn.innerText = "📥 تحميل القيود الشاملة"; }
    }
};
...
window.payDelayedBalance = async function (clientIdOverride, amountToPayOverride) {
    ...
    // 🚀 التقاط بيانات الشيكات إن وجدت في النافذة
    const chequeDateEl = document.getElementById('payOffChequeDate') || document.querySelector('input[name="cheque_date"]');
    const bankNameEl = document.getElementById('payOffBankName') || document.querySelector('input[name="bank_name"]');
    const chequeDate = chequeDateEl ? chequeDateEl.value : null;
    const bankName = bankNameEl ? bankNameEl.value : null;

    const debtIdInput = document.getElementById('payOffDebtId');
    const debtId = debtIdInput ? debtIdInput.value : null;
    ...
    if (amountInput && !amountToPayOverride) {
        ...
        if (maxBal > 0 && numAmountToPay > maxBal) {
            alert(`خطأ: لا يمكن سداد مبلغ أكبر من المديونية المتبقية لهذه المعاملة (${window.formatMoney ? window.formatMoney(maxBal) : maxBal}).`);
            return;
        }
    }

    if (!confirm(`تأكيد سداد مبلغ ${window.formatMoney ? window.formatMoney(numAmountToPay) : numAmountToPay}؟\n\n(سيتم التحديث في دفتر الأستاذ وكشوفات العميل آلياً)`)) return;

    const btn = document.querySelector('button[onclick="window.payDelayedBalance()"]');
    let originalText = "تأكيد السداد 💸";
    if (btn) { originalText = btn.innerText; btn.disabled = true; btn.innerText = "جاري التنفيذ ⏳..."; }
    ...
    try {
        ...
        // إذا كان debtId غير موجود (null/فاضي) سيفهم الباك إند أنه سداد شامل بنظام FIFO
        ...
        if (res.ok && result.success) {
            alert("✅ تم تسجيل السداد وتوليد القيود المحاسبية بنجاح.");
            ...
            if (typeof window.safeSystemRefresh === 'function') await window.safeSystemRefresh('client_consumptions');
